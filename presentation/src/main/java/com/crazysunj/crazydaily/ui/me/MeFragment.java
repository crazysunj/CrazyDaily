 ...
 */
@RuntimePermissions
public class MeFragment extends BaseFragment implements PermissionStorage {
    ...
    protected void initListener() {
        mAboutApp.setOnClickListener(v -> BrowserActivity.start(mActivity, getString(R.string.url_about_app)));
        mAboutMe.setOnClickListener(v -> BrowserActivity.start(mActivity, getString(R.string.url_about_me)));
        mUpdate.setOnClickListener(v -> MeFragmentPermissionsDispatcher.showUpdateWithPermissionCheck(this));
        mFeedback.setOnClickListener(v -> MeFragmentPermissionsDispatcher.showFeedbackWithPermissionCheck(this));
        mClearCache.setOnClickListener(v -> MeFragmentPermissionsDispatcher.clearCacheWithPermissionCheck(this));
        mHandleImg.setOnClickListener(v -> MeFragmentPermissionsDispatcher.handleImgWithPermissionCheck(this));
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (resultCode == Activity.RESULT_OK && requestCode == REQUEST_SELECT_PICTURE) {
            ...
            if (selectedUri != null) {
                PhotoActivity.start(mActivity, selectedUri.toString());
            } else {
                SnackbarUtil.show(mActivity, "选择图片失败");
            }
        }
    }
        ...
        new PgyUpdateManager.Builder()
                ...
                .setUpdateManagerListener(new UpdateManagerListener() {
                    ...
                    public void onNoUpdateAvailable() {
                        // 无更新
                        SnackbarUtil.show(mActivity, "已经是最新的哦");
                    }

                    @Override
                    public void onUpdateAvailable(AppBean appBean) {
                        SnackbarUtil.show(mActivity, "正在下载中");
                    ...
                    public void checkUpdateFailed(Exception e) {
                        //更新检测失败回调
                        SnackbarUtil.show(mActivity, "更新失败哦");
                    }
                })
    ...
    @NeedsPermission({Manifest.permission.READ_EXTERNAL_STORAGE, Manifest.permission.WRITE_EXTERNAL_STORAGE})
    void clearCache() {
        final boolean isSuccess = CacheUtil.cleanExternalCache(mActivity);
        SnackbarUtil.show(mActivity, "清除缓存" + (isSuccess ? "成功" : "失败"));
    }
